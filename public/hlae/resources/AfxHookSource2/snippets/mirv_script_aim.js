"use strict";
{
    ...
    let resolver = null;
    let lastTarget = null;
    const lookAnglesFromTo = (from, to, fallBackPitch, fallBackYaw) => {
    ...
    const aim = (e) => {
        ...
        if (null !== target) {
            lastTarget = target;
            const lookAngles = lookAnglesFromTo(new AdvancedfxMathVector3(e.currentView.x, e.currentView.y, e.currentView.z), new AdvancedfxMathVector3(target[0], target[1], target[2]), e.lastView.rX, e.lastView.rY);
            if (snapTo) {
                return {
                    rX: lookAngles.pitch,
                    rY: lookAngles.yaw,
            ...
            const sourceQuat = AdvancedfxMathQuaternion.fromQREulerAngles(AdvancedfxMathQREulerAngles.fromQEulerAngles(new AdvancedfxMathQEulerAngles(e.lastView.rX, e.lastView.rY, e.lastView.rZ)));
            let targetQuat = AdvancedfxMathQuaternion.fromQREulerAngles(AdvancedfxMathQREulerAngles.fromQEulerAngles(new AdvancedfxMathQEulerAngles(lookAngles.pitch, lookAngles.yaw, 0.0)));
            const dot = sourceQuat.dot(targetQuat);
            if (dot < 0)
                targetQuat = targetQuat.leftMul(-1.0);
            const interQuat = sourceQuat.slerp(targetQuat, e.absTime ? Math.max(0, Math.min((percentPerSecond / 100.0) * e.absTime, 1)) : 0);
    ...
    const fn = (args) => {
        ...
        if (2 <= argC) {
            ...
            if (arg1 === 'active') {
                ...
                mirv.message(`${arg0} active 0|1 - Whether aiming is active (1) or not (0).
                ...
                mirv.message(`${arg0} entityIndex <n> - Entity index to aim after (use ${arg0} list to get one). Use invalid index (i.e. -1) to deactivate re-targeting.
					Current value: ${index}
					`.dedent());
                return;
            }
                ...
                mirv.message(`${arg0} snapTo 0|1 - Whether to aim non-soft (1) or not (0).
                ...
                mirv.message(`${arg0} point abs <fX> <fY> <fZ> - Aim at absolute point in world coordinates.
                ...
                if (3 === argC) {
                    ...
                    if (arg2 === 'entities') {
                        ...
                        for (let i = 0; i < highest + 1; i++) {
                            ...
                            if (null !== entity) {
                                mirv.message(`${i} : ${entity.getClientClassName()} / ${entity.getDebugName()} / ${entity.getClassName()} | ${entity.getRenderEyeOrigin()} / ${entity.getRenderEyeAngles()}\n`);
                            }
                        }
                        return;
                    }
    ...
    // @ts-ignore
    if (mirv._mirv_script_aim !== undefined)
        mirv._mirv_script_aim.unregister();
    // @ts-ignore

"use strict";
{
    ...
    // @ts-ignore
    if (mirv._mirv_script_point !== undefined)
        mirv._mirv_script_point.unregister();
    const state = {
        prev: -1,
        current: 0,
        next: 1,
    ...
    const resolveData = (type, index, shouldSelect) => {
        let camPoint;
        switch (typeof index) {
            case 'number':
        ...
        if (type === 'all')
            mirv.exec([position, angles, fov, time].join('; '));
        if (shouldSelect)
            mirv.exec(`mirv_campath select #${camPoint.index} #${camPoint.index}`);
        if (state.debug) {
            mirv.message(`Resolved campath point:
    ...
    mirv._mirv_script_point = new AdvancedfxConCommand((args) => {
        ...
        if (2 <= argC) {
            ...
            let arg2 = argC >= 3 ? args.argV(2) : undefined;
            if (arg2) {
    ...
    // @ts-ignore
    mirv._mirv_script_point.register('mirv_script_point', 'Apply campath point properties to mirv_input');
}
